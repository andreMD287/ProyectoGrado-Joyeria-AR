...
| **Adapter** | `AndroidHandDetector`, `IosHandDetector`, `FaceDetectorDataSource`, `PoseDetectorDataSource` frente a `LandmarkDetector` | Plugins y APIs nativas incompatibles entre sí y entre SO | El dominio solo ve `LandmarkFrame` (ADR-15). Alternativa descartada: acoplar `TryOnScreen` a `hand_landmarker` (imposible en iOS). |
...
| ADR-08 | Overlay GLB sobre `CameraPreview` como render de prueba virtual | Valida anclaje anatómico sin exigir ARCore en el dispositivo de pruebas. **Superada por ADR-16 (2026-09-18):** cumplió su propósito —permitió cerrar anclaje, escala y orientación sin depender de ARCore— pero su render por WebView no puede dejar de verse superpuesto. Se conserva aquí porque explica por qué el pipeline de anclaje es independiente de ARCore, que sigue vigente | Forzar ARCore/ARKit para el anclaje corporal |
...
| ADR-14 | Fork propio de `hand_landmarker` (desde `v3.0.0`), fijado **por commit** en vez del paquete publicado | MediaPipe ya calcula `worldLandmarks` —los 21 puntos en metros, con origen en el centro geométrico de la mano— y el plugin publicado los **descartaba al serializar**, dejando solo los normalizados, cuya `z` es profundidad relativa en unidades de imagen. Sin reconstrucción métrica no hay forma de recuperar la orientación 3D real de la muñeca, que es el insumo de cualquier render que aspire a no verse superpuesto. El fork los expone y unifica los dos serializadores del plugin. **Comprobaciones previas a adoptarlo:** el `hand_landmarker.task` que trae el plugin es **byte a byte** el modelo oficial de Google (SHA256 idéntico al del bucket de MediaPipe); el plugin no declara permisos, no usa red ni ejecución dinámica, y son 422 líneas auditables; la inferencia proviene de `com.google.mediapipe:tasks-vision`, artefacto oficial de Google. Se fija el **commit** y no la rama porque `^3.0.0` aceptaba actualizaciones automáticas de un paquete con un solo mantenedor. | Vendorizar el plugin en el repositorio (metía un binario de 7,5 MB a la historia de git o a la cuota de LFS, que el propio CI señala como ajustada); reescribir el puente JNI desde cero (misma superficie y mismo modelo, sin ganancia); forkear `main` (3.1.0 exige Dart `^3.12` y el proyecto va en 3.11.5); conformarse con la `z` normalizada (no es métrica: no reconstruye orientación) |
| ADR-15 | El contrato del detector transporta **dos espacios de coordenadas** (`LandmarkFrame`) en lugar de una lista de landmarks | Los puntos de imagen y los métricos no son dos vistas del mismo dato: los primeros posicionan la joya sobre el preview, los segundos solo sirven para orientación 3D. Mezclarlos obligaría a cada consumidor a saber en qué espacio está leyendo, que es exactamente la clase de confusión que ya costó errores en este pipeline (§2.9). `LandmarkFrame` los lleva juntos pero separados, con la parte métrica vacía cuando el detector no la provee — hoy solo MediaPipe manos en Android, y quien la consuma debe degradar, no asumirla. El cambio atraviesa el contrato de plugin del microkernel (ADR-13) y la frontera del isolate (ADR-06): por eso es arquitectónico y no un detalle de implementación. | Añadir `x/y/z` métricos al propio `Landmark` (dos espacios en un tipo, sin forma de saber cuál se lee); un método aparte en el detector (dos llamadas por frame que hay que sincronizar entre sí); un canal paralelo solo para manos (rompe la uniformidad del contrato de plugin) |
| ADR-16 | Motor de render propio (`three_js`) en reemplazo de `model_viewer_plus` para la prueba virtual | `model_viewer_plus` renderiza el modelo desde **su propia cámara** dentro de un WebView y el resultado se compone como un *sprite* 2D sobre el preview: no existe relación de perspectiva con la extremidad real, por lo que la pieza se percibe superpuesta y los detalles asimétricos (un dije) caen del lado que no corresponde. Verificado en dispositivo el 2026-09-18; un recorte 2D se implementó y se comprobó insuficiente, porque ataca el síntoma y no la causa. `three_js` da **cámara en perspectiva propia** —que puede imitar la del teléfono— y **buffer de profundidad**, del que la oclusión sale como consecuencia y no como parche. **Verificado por spike en dispositivo** (Galaxy A15): renderiza con fondo transparente (`Settings(alpha: true, clearAlpha: 0)`) componiendo sobre widgets de Flutter, y carga 5 de los 7 GLB del catálogo. Además dibuja en un `Texture` de Flutter y no en una *platform view*, lo que elimina de paso la latencia de composición asíncrona del WebView. **Funciona con Flutter 3.41.7**, la versión que el CI fija, sin exigir subir el SDK. **Costos asumidos, medidos en el spike:** (a) arrastra compilación nativa con CMake+NDK, que el CI tendrá que instalar (`cmake;3.31.4`); (b) es pre-1.0 y de un solo mantenedor, con fallos propios de Android listados por el autor; (c) su cargador glTF rompe con `KHR_materials_specular` cuando Blender exporta `specularFactor` como entero (`0` en vez de `0.0`) — parche de una línea en `gltf_extensions.dart`, o reexportar las piezas; (d) el widget **cachea su tamaño en el primer build**, así que debe crearse con tamaño explícito una vez conocidas las restricciones, o falla con «Invalid dimensions». | **`model_viewer_plus` (statu quo):** descartado por la causa raíz de arriba — sin cámara propia ni profundidad no puede dejar de verse como calcomanía, y el WebView añade latencia. **`flutter_scene`:** el candidato más sólido en calidad (glTF, PBR, del equipo de Flutter), pero exige **Flutter ≥ 3.47** y el proyecto está fijado en 3.41.7 por el CI; subir el SDK es decisión de equipo y además **no es por rama** (cambia la instalación de toda la máquina), así que no se puede ensayar y revertir con git. Queda como plan B, aislable con FVM si algún día se adopta. **Depth API de ARCore/ARKit:** ya descartada en ADR-09 y en B6 por soporte limitado de profundidad en cámara frontal —2 de 3 categorías— y dependencia de hardware |
| ADR-17 | La geometría del **brazo** se estima desde la **mano**, con constantes antropométricas calibradas en dispositivo, y la oclusión usa un proxy geométrico de la extremidad | Ninguna fuente disponible mide el antebrazo, y las tres se descartaron con evidencia en `spikes/B7-motor-render`: **MediaPipe Hands** termina en la muñeca (21 puntos, el último es el landmark 0); **ML Kit Pose** devuelve 33 puntos a 15 Hz pero todos amontonados sobre la mano —los que marca como muñeca y codo caen en los dedos— y encima los reporta con `likelihood` **0,99**, de modo que esa métrica **no sirve de filtro de validez**; **la segmentación de persona** (`google_mlkit_selfie_segmentation` 0.11.0, última compatible con el SDK que fija el CI) cubre de máscara casi toda la escena —escritorio, ratón, suelo— porque está entrenada para *selfies*, y además corre a 2,5 Hz, cuatro veces por debajo del presupuesto de ADR-12. En consecuencia: el **eje** del antebrazo se extrapola prolongando el de la mano; su **ancho** sale de una proporción antebrazo/palma; y la **oclusión** se hace con un cono elíptico invisible que escribe profundidad sin pintar color, orientado con ese eje y achatado según la normal de la palma, acotado al hueco del aro para que no atraviese la pieza. La pieza se escala para que **su hueco abrace el brazo**, no para que su diámetro exterior mida algo: escalando por el exterior el hueco salía más estrecho que el antebrazo y la pieza era incapaz de rodearlo. **Limitaciones conocidas y medidas:** (a) la muñeca se dobla, así que el eje extrapolado trae error angular —medido con el overlay de diagnóstico, 95° estimados contra un antebrazo que bajaba hacia la derecha—, y el desvío lateral que produce es proporcional a `forearmOffset`, por eso baja de 0,45 a 0,20 y el desvío del ancla cae de 25 px a ~5; (b) las constantes antropométricas se calibraron sobre **una sola persona** y por juicio visual, porque la medición automática sobre captura se fragmenta justo donde la pieza cruza el brazo; (c) el ajuste del aro **no se dio por bueno**: el usuario lo reporta como no convincente al cierre del trabajo. | **Medir el brazo con segmentación o con pose** — descartadas arriba con datos. **Derivar la geometría de la reconstrucción métrica de MediaPipe** — descartada en ADR-14: de ella solo sirven las direcciones (su escala absoluta yerra ~45% y su ancho de palma varía un 65% al girar la mano). **Pedir la talla al usuario** — trasladaría al cliente un problema técnico y no resuelve ni el eje ni la oclusión |
...
| **Alta** | ~~Estimar `rollRadians` en `BraceletStrategy` para que la pulsera rote con el giro de la muñeca~~ → hecho: `rollRadians` sale del eje del antebrazo y el yaw de pronación se aplica en vivo al `<model-viewer>` | Cerrado; el yaw sigue siendo una aproximación 2D (sin profundidad real) |
| **Alta** | El ajuste del aro sobre el brazo **no convence al usuario** | Migración hecha y oclusión funcionando, pero el cierre de la pieza alrededor del antebrazo sigue sin darse por bueno. Topa con ADR-17: sin una fuente que mida el brazo, el eje y el ancho son estimaciones. Antes de seguir afinando constantes conviene decidir si se acepta como límite del alcance |
| **Alta** | Las constantes antropométricas están calibradas sobre **una sola muñeca** | `_limbToPalm` y la holgura del aro se ajustaron con un único usuario. Validar con otras personas antes de la sustentación, o el comportamiento sorprenderá en la demo |
| Media | Temblor residual de la pieza | Medido: la oscilación del eje bajó de 9,1° a 1,1° amortiguando la profundidad y cerrando filtros, pero el usuario lo sigue percibiendo. Queda un pico ocasional de posición (salto máximo de 0,128 del ancho del frame) que parece fallo puntual de detección, no ruido continuo |
| Media | `cartier.glb` se dibuja pequeño | Su caja envolvente no describe un aro —extensión [26,6 × 71,9 × 35,2] y tres mallas extra, probablemente el destornillador de la pieza—, así que ni el diámetro ni el eje del aro se pueden deducir de ella. Reexportar con solo la joya |
| Media | Aretes y collares siguen sin oclusión | No tienen reconstrucción métrica, así que no hay eje 3D con el que orientar un proxy |
| **Alta** | Aprovechar los landmarks métricos ya disponibles (ADR-14/15) para estimar una orientación 3D real de la muñeca | Hoy la pronación se aproxima por escorzo 2D; los datos métricos llegan pero todavía no los consume ninguna estrategia |
| Media | Oclusión por objetos externos (cabello sobre el arete, dedos sobre la muñeca) | Caso que estudia B6 — requiere segmentación por frame y medir su costo contra ADR-12 |
...
| Hecho (doc) | Tres ejes de dominio, ADR 01–17 (microkernel §2.2.1, landmarks métricos ADR-14/15, motor de render ADR-16, geometría del brazo ADR-17), escenarios EC-01…06, rename Fallback | Cubierto en este SDD |
